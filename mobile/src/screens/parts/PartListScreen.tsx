...
export default function PartListScreen({ route, navigation }: any) {
  ...
  const load = useCallback(async () => {
    try {
      const res = await partsApi.getAll(vehicleId);
      setParts(res.data);
    } catch (e) {
      console.log(e);
    }
  ...
  const handleEdit = (record: any) => {
    navigation.navigate('AddPart', { vehicleId, record });
  };
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            subtitle="Tap + to add a component or spare part"
          />
        }
        renderItem={({ item }) => (
          <TouchableOpacity activeOpacity={0.8} onPress={() => handleEdit(item)}>
            <Card style={styles.card}>
              <View style={styles.cardRow}>
                ...
                {item.cost ? <Text style={styles.cardCost}>₹{item.cost.toLocaleString()}</Text> : null}
              </View>
