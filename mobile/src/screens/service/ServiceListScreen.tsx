...
export default function ServiceListScreen({ route, navigation }: any) {
  ...
  const load = useCallback(async () => {
    try { const res = await servicesApi.getAll(vehicleId); setRecords(res.data); } catch (e) { console.log(e); }
  }, [vehicleId]);
  ...
  const handleDelete = (id: string) => {
    Alert.alert('Delete', 'Delete this service record?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => { await servicesApi.delete(vehicleId, id); load(); }},
    ]);
  };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.addBtn} onPress={() => navigation.navigate('AddService', { vehicleId })}>
          <Ionicons name="add" size={22} color={colors.textOnPrimary} />
        </TouchableOpacity>
      </View>
      <FlatList data={records} keyExtractor={(item) => item.id} contentContainerStyle={styles.list}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.primary} />}
        ListEmptyComponent={<EmptyState icon="build-outline" title="No service records" subtitle="Tap + to log a service" />}
        renderItem={({ item }) => (
          <TouchableOpacity onLongPress={() => handleDelete(item.id)}>
            <Card style={styles.card}>
              <View style={styles.cardRow}>
                <View style={[styles.cardIcon, { backgroundColor: colors.service + '15' }]}><Ionicons name="build" size={18} color={colors.service} /></View>
                <View style={{ flex: 1 }}>
                  ...
                  {item.serviceCenter && <Text style={styles.cardSub}>{item.serviceCenter}</Text>}
                </View>
